...
public final class ConfirmationHelper {
    ...
    public static void deliver(
        ...
        if (CHANNEL_DEVICE.equals(preference)) {
        ...
        if (CHANNEL_EMAIL.equals(preference)) {
            ...
            try {
                ...
                send.putExtra(
                    ...
                    context.getString(R.string.confirmation_email_subject, event.getTitle())
                );
                send.putExtra(Intent.EXTRA_TEXT, body);
                send.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                context.startActivity(
                    Intent.createChooser(send, context.getString(R.string.confirmation_send_email))
        ...
        if (CHANNEL_SMS.equals(preference)) {
        ...
        // Safe fallback for unknown preference values.
        postDeviceNotification(context, event.getTitle(), body);
    }
    ...
    private static String buildBody(Event event) {
        String when = event.getTime();
        if ((when == null || when.trim().isEmpty()) && event.getDateTimeMillis() > 0) {
            when = DateFormat.getDateTimeInstance().format(new Date(event.getDateTimeMillis()));
        }
